...
static void ms5611_reset(void)
{
    MS5611_SPI_CS_L;
    // delay_ms(1);
    SPI_TansmissionReceiveByte(CMD_RESET);
    delay_ms(3);/*官方代码推荐*/
    MS5611_SPI_CS_H;
    // delay_ms(1);
}
...
static void ms5611_Readprom(ms5611Prom_t* prom_buff)
{
    ...
    for (i = 0; i < 8;i++)
    {
        MS5611_SPI_CS_L;
        // delay_ms(1);
        SPI_TansmissionReceiveByte(CMD_PROM_RD + 2 * i);
        prom_buff->prom[i]=SPI_TansmissionReceiveByte(0x00);/*MSB*/
        prom_buff->prom[i] <<= 8;
        prom_buff->prom[i] |= SPI_TansmissionReceiveByte(0x00); /*LSB*/
        MS5611_SPI_CS_H;
        // delay_ms(1);
    }
}
...
static uint32_t ms5611_ReadADC(uint8_t adc, uint8_t osr)
{
    ...
    SPI_TansmissionReceiveByte(CMD_ADC_CONV + adc + osr);
    ...
    // delay_ms(1);
    /*读取ADC数据*/
    SPI_TansmissionReceiveByte(CMD_ADC_READ);
    ret = SPI_TansmissionReceiveByte(0x00); /*MSB*/
    ret <<= 8;
    ret |= SPI_TansmissionReceiveByte(0x00);
    ret <<= 8;
    ret |= SPI_TansmissionReceiveByte(0x00);
    MS5611_SPI_CS_H;
    // delay_ms(1);
